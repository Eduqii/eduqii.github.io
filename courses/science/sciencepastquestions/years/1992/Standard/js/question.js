

//Array of Objects
const quiz = [
 {
		question: 'Which of the following is true about matter? It has',
options: ['Weight and direction',
 'Weight only',
 'Weight and volume',
 'Volume and direction',
 'Volume only'],
answer: 2
},
{
question: 'The combining power of an element is called',
options: ['ion',
 'electron',
 'atom',
 'valency',
 'proton'],
answer: 3
},
{
question: 'Why is it bad to throw faeces into rivers? Because',
options: ['Other people collect drinking water from them',
 'It changes the colour of the river',
 'It causes hardness of the water',
 'The faeces cannot dissolve in the water',
 'The riverbanks will be dirty'],
answer: 0
},
{
question: 'Which of the following methods will be most suitable for separating a mixture of iron filings and sulphur powder?',
options: ['Winnowing',
 'Decanting',
 'Magnetization',
 'Evaporation',
 'Distillation'],
answer: 2
},
{
question: 'During athletics, a boy slipped on a banana peel on the ground and fell. This is because',
options: ['There was no friction',
 'The ground was hard',
 'The banana peel was soft',
 'The ground was sandy',
 'The frictional force was too great'],
answer: 0
},
{
question: 'Which of the following is a rhizome?',
options: ['Potato',
 'Ginger',
 'Onion',
 'Cocoyam',
 'Cassava'],
answer: 1
},
{
question: 'How many different elements are present in the compound CuCl2?',
options: ['Five ',
 'Four ',
 'Three ',
 'Two ',
 'One'],
answer: 3
},
{
question: 'Which of the following energy changes takes place when a church bell is tolled?',
options: ['Potential energy to kinetic energy',
 'Kinetic energy to sound energy',
 'Potential energy to sound energy',
 'Chemical energy to heat energy',
 'Sound energy to Kinetic energy'],
answer: 1
},
{
question: 'Living things are different from non-living things because living things',
options: ['are made up of protoplasm',
 'do not have definite forms',
 'take in liquid foods',
 'have chlorophyll',
 'vary widely in form and size'],
answer: 0
},
{
question: 'Which of the following gases enable burning to take place?',
options: ['Oxygen',
 'Carbon dioxide',
 'Hydrogen',
 'Ammonia',
 'Nitrogen'],
answer: 0
},
{
question: 'The work done when a weight of 40 N is lifted up vertically through a distance of 10m is',
options: ['400 J ',
 '50 J ',
 '30 J ',
 '4 J ',
 '0.25 J'],
answer: 0
},
{
question: 'Which of the following products is not excreted by the skin?',
options: ['Water',
 'Sweat',
 'Urea',
 'Common salt',
 'saliva'],
answer: 4
},
{
question: 'An element is formed by the combination of',
options: ['an atom and a compound',
 'a compound and a mixture',
 'an atom and a mixture',
 'a molecule and a compound',
 'the same kind of atoms'],
answer: 4
},
{
question: 'Energy is measured in',
options: ['Kelvin',
 'Joules',
 'Watts',
 'Metres',
 'Amperes'],
answer: 1
},
{
question: 'Which of the following is not a characteristic of all livingthings?',
options: ['Transpiration',
 'Growth',
 'Reproduction',
 'Feeding',
 'Respiration'],
answer: 0
},
{
question: 'Which of the following substances is a mixture?',
options: ['Sodium chloride',
 'Sulphur dioxide',
 'Sugar solution',
 'Carbon dioxide',
 'Ammonium chloride'],
answer: 2
},
{
question: 'The best reason why a cooking pan should have a wooden handle is that, wood is a',
options: ['good conductor of heat ',
 'bad conductor of heat ',
 'good radiator of heat',
 'bad radiator of heat',
 'good reflector of heat'],
answer: 1
},
{
question: 'For healthy growth, the food we eat must contain',
options: ['fat, oils and proteins',
 'important nutrients in the right proportions',
 'a good proportion of carbohydrates and proteins',
 'mineral salts, vitamins and water',
 'mineral salts in right proportion'],
answer: 1
},
{
question: 'When the testa of a soaked bean is removed, the seed is seen to be made up of mainly',
options: ['micropyles',
 'hilum',
 'radicle',
 'embryo',
 'cotyledons'],
answer: 4
},
{
question: 'Which of the following human activities causes pollution of the environment?',
options: ['Cutting down timber in the forest for firewood',
 'Overgrazing of the field by cattle',
 'Burning of coal and petrol in industries',
 'Building dams across rivers',
 'Destruction of diseased crops'],
answer: 2
},
{
question: 'Which of the following explains why no work is done when a push is made against a firm wall?',
options: ['The force acts in the opposite direction',
 'The wall does not move',
 'The force is in the same direction',
 'The material of the wall is strong',
 'The force is in horizontal direction'],
answer: 1
},
{
question: 'The process by which a hot saturated solution is cooled to obtain some of the solute is called',
options: ['condensation',
 'evaporation',
 'crystallization',
 'distillation',
 'sedimentation'],
answer: 2
},
{
question: 'Which of the following diseases is caused by a fungus?',
options: ['Typhoid',
 'Leprosy',
 'Ringworm',
 'Measles',
 'Dysentery'],
answer: 2
},
{
question: 'Stem cutting is used in propagating cassava because it',
options: ['gives rise to roots and leaves',
 'stores food in the stem',
 'gives rise to big tubers',
 'gives rise to small tubers',
 'stores food in the leaves'],
answer: 0
},
{
question: 'Which of the following methods is not away of preventing malaria parasites from person to person?',
options: ['Sleeping in mosquito nets',
 'Sleeping outside in the open',
 'Draining all stagnant pools of water around our homes',
 'Taking anti-malaria drugs weekly',
 'Burying all emptycans that hold water in our surroundings'],
answer: 1
},
{
question: 'Faeces is the',
options: ['excretory products resulting from body activities',
 'food which is digested',
 'food in the mouth which is removed from the body',
 'unwanted product of digestion which is removed from the body',
 'excess food which is stored in the body'],
answer: 3
},
{
question: 'Which of the following explains why a wet cloth dries faster on a dry day than on a humid day?',
options: ['The rate of evaporation is very high during a dry day',
 'There is much water vapour in the atmosphere during a dry day',
 'It is very misty during a dry day',
 'there is no evaporation during a humid day',
 'The sun does not set during a humid day'],
answer: 0
},
{
question: 'Urinating in rivers is a common manner of spreadingdiseases such as',
options: ['cholera',
 'bilharzia',
 'malaria',
 'measles',
 'Typhoid'],
answer: 1
},
{
question: 'Which of the following is not away of preventing soil erosion?',
options: ['Planting trees on bare soil.',  'Clearing trees along slopes',
 'Planting grasses along slopes',
 'Ploughing across slopes',
 'Preventing bush fire'],
answer: 1
},
{
question: 'Atmospheric pressure is measured with',
options: ['an anemometer',
 'athermometer',
 'agalvanometer',
 'abarometer',
 'avoltmeter'],
answer: 3
},
{
question: 'Which of the following is not matter?',
options: ['Iced block',
 'Air',
 'Light',
 'Smoke',
 'Stone'],
answer: 2
},
{
question: 'Regular brushing of teeth is primarily done in order to',
options: ['keep the mouth fresh',
 'clear away food particles from the teeth',
 'remove bad smelling the mouth',
 'kill bacteria in the mouth',
 'control the flow of saliva'],
answer: 1
},
{
question: 'On heating a certain solid in a dry container, all the substance was lost without passing through the liquid state. This means the',
options: ['solid can easily melt',
 'solid is an alloy',
 'solid cannot stand heat',
 'solid particles are held together by weak forces',
 'solid undergoes sublimation'],
answer: 4
},
{
question: 'Which of the following forces tends to pull forces towards the centre of the earth?',
options: ['Electrostatic force',
 'Magnetic force',
 'Force of gravity',
 'Force of attraction',
 'Centrifugal force'],
answer: 2
},
{
question: ' Which of the following diseases will result from eating improperlycooked meat?',
options: ['Tapeworm infestation',
 'Guinea worm infestation',
 'Small pox',
 'Leprosy',
 'Measles'],
answer: 0
},
{
question: 'The angle between a ray of light and the surface it strikes is 30°. Calculate the angle of incidence.',
options: ['210° ',
 '120° ',
 '90° ',
 '60° ',
 '30°'],
answer: 3
},
{
question: 'The Central Nervous System consists of',
options: ['the brain and the spinal cord',
 'the brain and the ears',
 'the spinal cord and eyes',
 'the brain, eyes and ears',
 'eyes, ears, skin and spinal cord'],
answer: 0
},
{
question: 'Which of the following classes of food can be broken down into amino acids?',
options: ['Carbohydrates',
 'Vitamins',
 'Mineral salts',
 'Fats and oils',
 'Proteins'],
answer: 4
},
{
question: 'Which of the following is the name of the darkest part of a shadow formed when a large source of light falls on an object?',
options: ['Partial eclipse',
 'Lunar eclipse',
 'Total eclipse',
 'Penumbra',
 'Umbra'],
answer: 4
},
{
question: 'Which of the following is evidence that particles in gases move faster than those in liquids and solids?' + '<br />' + 
'I. The scent of a perfume fills a room when its container is opened ' + '<br />' +
'II. Smoke spreads over a large area as burning takes place' + '<br />' + 
'III. The wind blows fast when it is about to rain',
options: ['I only',
 'III only',
 'II only',
 'I and IIonly',
 'I,II and III'],
answer: 3
}
]