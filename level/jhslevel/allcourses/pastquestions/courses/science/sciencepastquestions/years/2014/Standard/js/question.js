

//Array of Objects
const quiz = [
  {
		question: 'Which of the following types of teeth is used by mammals to tear food material?',
options: ['Canines',
 'Incisors',
 'Molars',
 'Premolars'],
answer: 0
},
{
question: 'The eclipse formed when the moon comes between the sun and the earth is known as',
options: ['annular eclipse',
 'lunar eclipse',
 'solar eclipse',
 'total eclipse'],
answer: 2
},
{
question: 'The element with the chemical symbol S is',
options: ['silicon',
 'silver',
 'sodium',
 'sulphur'],
answer: 3
},
{
question: 'The term leaching in soils refers to',
options: ['accumulation of organic matter',
 'decomposition of plant material.',
 'fixation of nitrogen',
 'removal of soil nutrients by water.'],
answer: 3
},
{
question: 'Which of the following statements about a plant cell is correct? It',
options: ['does not have a nucleus',
 'contains large vacuoles',
 'is surrounded by the cell membrane only',
 'does not have a definite shape'],
answer: 1
},
{
question: 'An example of a derived quantity is',
options: ['length',
 'mass',
 'temperature',
 'volume'],
answer: 3
},
{
question: 'Solid non-metals normally break into pieces when hammered because they are',
options: ['brittle',
 'ductile',
 'lustrous',
 'malleable'],
answer: 0
},
{
question: 'Young rabbits are called',
options: ['bunnies',
 'cubs',
 'fingerlings',
 'kids'],
answer: 0
},
{
question: 'Producers in an ecosystem are plants that',
options: ['attract insects',
 'feed on other plants',
 'feed on dead materials',
 'manufacture their own food.'],
answer: 3
},
{
question: 'The form of energy produced from the nucleus of an atom is termed',
options: ['chemical energy',
 'mechanical energy',
 'nuclear energy',
 'thermal energy'],
answer: 2
},
{
question: 'Which of the following elements is a semi-metal?',
options: ['Carbon',
 'Nitrogen',
 'Silicon',
 'Sodium'],
answer: 2
},
{
question: 'A system of farming that leads to continuous destruction of virgin forest is',
options: ['mixed farming',
 'mixed cropping',
 'pastoral farming',
 'shifting farming'],
answer: 3
},
{
question: 'The by-products of respiration are',
options: ['carbon dioxide and heat',
 'carbon dioxide and water',
 'oxygen and heat',
 'oxygen and water.'],
answer: 1
},
{
question: 'Which of the following statements about a force are correct? It' + '<br />' + '<br />' +
'I is measured in newtons' + '<br />' + 
'II is measured in newton-metre' + '<br />' + 
'III can start a motion' + '<br />' + 
'IV can change the direction of a moving body.',
options: ['I and II only',
 'I and III only',
 'I, III and IV only',
 'I, II, III and IV'],
answer: 2
},
{
question: 'Which of the following substances is a solid-gas mixture?',
options: ['Lather',
 'Bronze',
 'Steel',
 'Smoke'],
answer: 3
},
{
question: 'Weeds on a school farm could be controlled by',
options: ['handpicking',
 'mowing',
 'ploughing',
 'tilling'],
answer: 0
},
{
question: 'An example of a non-living tissue used in osmosis experiment is',
options: ['cellophane',
 'filter paper',
 'potato',
 'polythene'],
answer: 0
},
{
question: 'Kerosene is poured on the surface of a pond in order to',
options: ['make it unsafe for human consumption',
 'make it safe for animal consumption',
 'increase the surface tension',
 'break the surface tension'],
answer: 3
},
{
question: 'Which of the following processes is used to separate insoluble solids from liquids?',
options: ['Crystallization',
 'Evaporation',
 'Filtration',
 'Sublimation'],
answer: 2
},
{
question: 'The process of removing unproductive poultry birds from a flock is referred to as',
options: ['candling',
 'culling',
 'dehorning',
 'drenching'],
answer: 1
},
{
question: 'Which of the following pairs of diseases can be spread easily when food is exposed to houseflies?',
options: ['Dysentery and malaria',
 'Malaria and tuberculosis',
 'Dysentery and cholera',
 'Cholera and tuberculosis'],
answer: 1
},
{
question: 'The pressure in fluids',
options: ['acts differently in all directions',
 'acts upwards at any point',
 'decreases with depth',
 'increases with depth'],
answer: 3
},
{
question: 'Which of the following processes involve a change in the state of matter from liquid to solid?',
options: ['Boiling',
 'freezing',
 'Canning',
 'Smoking'],
answer: 1
},
{
question: 'Which of the following food items contains the highest amount of dietary fibre?',
options: ['Beans',
 'Bread',
 'Cabbage',
 'Egg'],
answer: 0
},
{
question: 'An example of a disease vector is',
options: ['earthworm',
 'liver fluke',
 'tapeworm',
 'tick'],
answer: 3
},
{
question: 'Which of the following devices is made of semiconductor?',
options: ['Capacitor',
 'Inductor',
 'Resistor',
 'Transistor'],
answer: 3
},
{
question: 'Which of the following statements about the properties of water are correct? It' + '<br />' + '<br />' + 
'I. is colourless and tasteless' + '<br />' + 
'II. is neutral to litmus paper' + '<br />' + 
'III. turns red litmus paper blue' + '<br />' + 
'IV. is a universal solvent',
options: ['I and II only',
 'I and III only',
 'I, II and IV only',
 'I, III and IV only'],
answer: 2
},
{
question: 'The study of the soil profile of an area helps the farmer to',
options: ['control weed growth',
 'determine the soil temperature',
 'determine the types of crop to grow.',
 'know the pesticides to use.'],
answer: 2
},
{
question: 'When a green leaf is placed in a test tube containing ethanol and heated over a water bath the leaf',
options: ['becomes soft',
 'changes colour to brown',
 'changes colour to blue-black',
 'is decolourized.'],
answer: 3
},
{
question: 'Which of the following sources of energy is / are renewable?' + '<br />' + '<br />' + 
'I. Solar ' + 
' II. Crude oil ' +
' III. Wind',
options: ['I only',
 'I and II only',
 'I and III only',
 'I, II and III'],
answer: 2
},
{
question: 'Which of the following substances is a compound?',
options: ['Hydrogen',
 'Nitrogen',
 'Oxygen',
 'Water'],
answer: 3
},
{
question: 'A disease-causing organism that is most difficult to control in crop production is',
options: ['bacterium',
 'fungus',
 'nematode',
 'virus'],
answer: 0
},
{
question: 'Which of the following organisms is/ are multicellular?' + '<br />' + '<br />' + 
'I. Amoeba ' +
' II. Paramecium ' +
' III. Onion',
options: ['I only',
 'III only',
 'I and III only',
 'II and III only'],
answer: 1
},
{
question: 'In electronic circuits, LEDs are used to indicate the absence or presence of',
options: ['emitter and collector',
 'electric current',
 'p-n junction',
 'voltage source.'],
answer: 1
},
{
question: 'Steel is an alloy of iron and',
options: ['aluminium',
 'carbon',
 'silicon',
 'gold'],
answer: 1
},
{
question: 'Soil aeration could be improved through',
options: ['mulching',
 'irrigation',
 'soil drainage',
 'fertilizer application'],
answer: 3
},
{
question: 'Which of the following pairs of structures are part of the respiratory system of humans?',
options: ['Fallopian tube and alveoli',
 'Pharynx and oesophagus',
 'Trachea and alveoli',
 'Trachea and duodenum'],
answer: 2
},
{
question: 'Which of the following effects is not a result of illegal connection of electricity? It can',
options: ['lead to fire outbreak',
 'lead to frequent power cut-off',
 'increase the flow of current in the supply chain.',
 'cause damage to electrical appliance'],
answer: 2
},
{
question: 'Which of the following statements about the scientific method is / are correct? It provides' + '<br />' + '<br />' + 
'I. logical procedure for arriving at knowledge.' + '<br />' + 
'II. knowledge that can be verified' + '<br />' + 
'III. knowledge that can never be changed',
options: ['I only',
 'I and II only',
 'I and III only',
 'II and III only'],
answer: 1
},
{
question: 'An entrepreneur’s decision as to what to produce is a/ an',
options: ['co-ordinating function',
 'organizing function',
 'planning function',
 'supervising function.'],
answer: 2
}
]