

//Array of Objects
const quiz = [
  {
  	question: 'SECTION A COMPREHENSION' + '<br />' +
'<b>' + 'Read the following passages carefully and answer the questions which follow' + '</b>' + '<br />' + '<br />' +
'PASSAGE I' + '<br />' + '<br />'  +
'<i>' + 'Njoman and Putu then went to a quiet spot on the path where it crossed a small stream. A few women were bathing in the stream; some were washing clothes. They joked with Njoman, for they knew why he was there. Soon they saw Ragini approaching with a basket of fruits and vegetables on her head.' +
'„Hello, Njoman,‟ she said, pretending not to know why he waited there with his best friend. „Hello Ragini. What did you buy in the market?‟ „Mostly fruits for rudjaks‟ (spicy salad). „Wait, Ragini‟, Njoman said as he stood up. „Putu will carry them for you. It is time for us to go off together and marry‟. Putu took the basket; Njoman took Ragini by the hand and said, „We shall go to my cousin‟s house in the next village‟.' +
'As they were walking off, Ragini turned to the women and feigned distress. „Njoman is taking me away. What can I do?‟ In this way, according to Balinese tradition, Ragini submitted to her abductor.' + '</i>' + '<br />' + '<br />' +  
' Njoman was on the path because he wanted to.....',
options: ['cross the stream',
 'bath in the stream',
 'spy on the woman',
 'meet Ragini',
 'meet Putu'],
answer: 3
},
{
question: 'SECTION A COMPREHENSION' + '<br />' +
'<b>' + 'Read the following passages carefully and answer the questions which follow' + '</b>' + '<br />' + '<br />' +
'PASSAGE I' + '<br />' + '<br />'  +
'<i>' + 'Njoman and Putu then went to a quiet spot on the path where it crossed a small stream. A few women were bathing in the stream; some were washing clothes. They joked with Njoman, for they knew why he was there. Soon they saw Ragini approaching with a basket of fruits and vegetables on her head.' +
'„Hello, Njoman,‟ she said, pretending not to know why he waited there with his best friend. „Hello Ragini. What did you buy in the market?‟ „Mostly fruits for rudjaks‟ (spicy salad). „Wait, Ragini‟, Njoman said as he stood up. „Putu will carry them for you. It is time for us to go off together and marry‟. Putu took the basket; Njoman took Ragini by the hand and said, „We shall go to my cousin‟s house in the next village‟.' +
'As they were walking off, Ragini turned to the women and feigned distress. „Njoman is taking me away. What can I do?‟ In this way, according to Balinese tradition, Ragini submitted to her abductor.' + '</i>' + '<br />' + '<br />' +  
'How did Ragini behave when she was invited by Njoman?',
options: ['She pretended she was in great pain and misery. ',
 'She talked to the women furiously',
 'She just walked off with her basket of fruits',
 'She greeted the women noisily',
 'She danced about with joy'],
answer: 0
},
{
question: 'SECTION A COMPREHENSION' + '<br />' +
'<b>' + 'Read the following passages carefully and answer the questions which follow' + '</b>' + '<br />' + '<br />' +
'PASSAGE I' + '<br />' + '<br />'  +
'<i>' + 'Njoman and Putu then went to a quiet spot on the path where it crossed a small stream. A few women were bathing in the stream; some were washing clothes. They joked with Njoman, for they knew why he was there. Soon they saw Ragini approaching with a basket of fruits and vegetables on her head.' +
'„Hello, Njoman,‟ she said, pretending not to know why he waited there with his best friend. „Hello Ragini. What did you buy in the market?‟ „Mostly fruits for rudjaks‟ (spicy salad). „Wait, Ragini‟, Njoman said as he stood up. „Putu will carry them for you. It is time for us to go off together and marry‟. Putu took the basket; Njoman took Ragini by the hand and said, „We shall go to my cousin‟s house in the next village‟.' +
'As they were walking off, Ragini turned to the women and feigned distress. „Njoman is taking me away. What can I do?‟ In this way, according to Balinese tradition, Ragini submitted to her abductor.' + '</i>' + '<br />' + '<br />' +  
'Which of the following is not true according to the passage?',
options: ['The women teased Njoman',
 'Putu held Ragini‟s hand',
 'All the women definitely knew his reason for coming',
 'Njoman waited for sometime before Ragini arrived',
 'Njoman had a secret plan to follow'],
answer: 1
},
{
question: 'SECTION A COMPREHENSION' + '<br />' +
'<b>' + 'Read the following passages carefully and answer the questions which follow' + '</b>' + '<br />' + '<br />' +
'PASSAGE I' + '<br />' + '<br />'  +
'<i>' + 'Njoman and Putu then went to a quiet spot on the path where it crossed a small stream. A few women were bathing in the stream; some were washing clothes. They joked with Njoman, for they knew why he was there. Soon they saw Ragini approaching with a basket of fruits and vegetables on her head.' +
'„Hello, Njoman,‟ she said, pretending not to know why he waited there with his best friend. „Hello Ragini. What did you buy in the market?‟ „Mostly fruits for rudjaks‟ (spicy salad). „Wait, Ragini‟, Njoman said as he stood up. „Putu will carry them for you. It is time for us to go off together and marry‟. Putu took the basket; Njoman took Ragini by the hand and said, „We shall go to my cousin‟s house in the next village‟.' +
'As they were walking off, Ragini turned to the women and feigned distress. „Njoman is taking me away. What can I do?‟ In this way, according to Balinese tradition, Ragini submitted to her abductor.' + '</i>' + '<br />' + '<br />' +  
'The expression pretending not to know means',
options: ['not knowing what to do',
 'behaving well after knowing',
 'behaving as if everybody knew what to say',
 'behaving as if one did not know something was happening',
 'not knowing how to behave.'],
answer: 3
},
{
question: 'SECTION A COMPREHENSION' + '<br />' +
'<b>' + 'Read the following passages carefully and answer the questions which follow' + '</b>' + '<br />' + '<br />' +
'PASSAGE I' + '<br />' + '<br />'  +
'<i>' + 'Njoman and Putu then went to a quiet spot on the path where it crossed a small stream. A few women were bathing in the stream; some were washing clothes. They joked with Njoman, for they knew why he was there. Soon they saw Ragini approaching with a basket of fruits and vegetables on her head.' +
'„Hello, Njoman,‟ she said, pretending not to know why he waited there with his best friend. „Hello Ragini. What did you buy in the market?‟ „Mostly fruits for rudjaks‟ (spicy salad). „Wait, Ragini‟, Njoman said as he stood up. „Putu will carry them for you. It is time for us to go off together and marry‟. Putu took the basket; Njoman took Ragini by the hand and said, „We shall go to my cousin‟s house in the next village‟.' +
'As they were walking off, Ragini turned to the women and feigned distress. „Njoman is taking me away. What can I do?‟ In this way, according to Balinese tradition, Ragini submitted to her abductor.' + '</i>' + '<br />' + '<br />' +  
'The expression feigned distress means',
options: ['fainted upon seeing the women',
 'pretended to be upset and having pain',
 'pretended to be very hungry on seeing the fruits',
 'fell down with flushed face',
 'stood still and watched the women'],
answer: 1
},
{
question: 'SECTION A COMPREHENSION' + '<br />' +
'<b>' + 'Read the following passages carefully and answer the questions which follow' + '</b>' + '<br />' + '<br />' +
'PASSAGE I' + '<br />' + '<br />'  +
'<i>' + 'Njoman and Putu then went to a quiet spot on the path where it crossed a small stream. A few women were bathing in the stream; some were washing clothes. They joked with Njoman, for they knew why he was there. Soon they saw Ragini approaching with a basket of fruits and vegetables on her head.' +
'„Hello, Njoman,‟ she said, pretending not to know why he waited there with his best friend. „Hello Ragini. What did you buy in the market?‟ „Mostly fruits for rudjaks‟ (spicy salad). „Wait, Ragini‟, Njoman said as he stood up. „Putu will carry them for you. It is time for us to go off together and marry‟. Putu took the basket; Njoman took Ragini by the hand and said, „We shall go to my cousin‟s house in the next village‟.' +
'As they were walking off, Ragini turned to the women and feigned distress. „Njoman is taking me away. What can I do?‟ In this way, according to Balinese tradition, Ragini submitted to her abductor.' + '</i>' + '<br />' + '<br />' +  
'The word abductor means',
options: ['a thief',
 'an armed robber',
 'a hijacker ',
 'a kidnapper ',
 'a murderer'],
answer: 3
},
{


question: '<b>' + 'Read Passage II carefully and answer the question which follow' + '</b>' + '<br />' + '<br />' + 'PASSAGE II' + '<br />' + '<br />' +
'<i>' + 'Apart from foods which supply us with energy, we need certain substances called protein to help us grow, and when we are fully grown, to maintain our strength. These proteins are found in meat, fish, eggs, milk, green vegetables and to a much lesser extent in grains like millet, wheat, guinea corn, rice, etc.' +
'Children fed chiefly on roots will, therefore, stop growing. They often get very ill and die while children who are given milk and eggs grow well and live longer.' +
'However, cow milk is expensive in West Africa and in many places there is a wrong tradition about eggs. Some old people say that if eggs are given to children they become liars. This is not true. Eggs do not make children either tell truth or lie. They are simply good food which will help the child to grow well. Also, the tradition that boys become thieves when they eat meat is not true. Both are bad traditions which have been repeated in some villages from one generation to another.' +
'So, the intelligent mother who wishes to bring up healthy children must discard them.' + '</i>' + '<br />' + '<br />' +
'According to the passage, proteins ………………',
options: ['make us well satisfied',
 'help us grow well',
 'fill our bodies',
 'give us energy',
 'help to maintain our tradition'],
answer: 1
},
{
question: '<b>' + 'Read Passage II carefully and answer the question which follow' + '</b>' + '<br />' + '<br />' + 'PASSAGE II' + '<br />' + '<br />' +
'<i>' + 'Apart from foods which supply us with energy, we need certain substances called protein to help us grow, and when we are fully grown, to maintain our strength. These proteins are found in meat, fish, eggs, milk, green vegetables and to a much lesser extent in grains like millet, wheat, guinea corn, rice, etc.' +
'Children fed chiefly on roots will, therefore, stop growing. They often get very ill and die while children who are given milk and eggs grow well and live longer.' +
'However, cow milk is expensive in West Africa and in many places there is a wrong tradition about eggs. Some old people say that if eggs are given to children they become liars. This is not true. Eggs do not make children either tell truth or lie. They are simply good food which will help the child to grow well. Also, the tradition that boys become thieves when they eat meat is not true. Both are bad traditions which have been repeated in some villages from one generation to another.' +
'So, the intelligent mother who wishes to bring up healthy children must discard them.' + '</i>' + '<br />' + '<br />' +
'Which of the following statements is not true according to the passage?',
options: ['Children must eat roots only',
 'Old people think meat makes boys thieves',
 'Children who are given milk and eggs will grow well',
 'The intelligent mother has to do away with some bad traditions',
 'Grains do not contain much protein.'],
answer: 0
},
{
question: '<b>' + 'Read Passage II carefully and answer the question which follow' + '</b>' + '<br />' + '<br />' + 'PASSAGE II' + '<br />' + '<br />' +
'<i>' + 'Apart from foods which supply us with energy, we need certain substances called protein to help us grow, and when we are fully grown, to maintain our strength. These proteins are found in meat, fish, eggs, milk, green vegetables and to a much lesser extent in grains like millet, wheat, guinea corn, rice, etc.' +
'Children fed chiefly on roots will, therefore, stop growing. They often get very ill and die while children who are given milk and eggs grow well and live longer.' +
'However, cow milk is expensive in West Africa and in many places there is a wrong tradition about eggs. Some old people say that if eggs are given to children they become liars. This is not true. Eggs do not make children either tell truth or lie. They are simply good food which will help the child to grow well. Also, the tradition that boys become thieves when they eat meat is not true. Both are bad traditions which have been repeated in some villages from one generation to another.' +
'So, the intelligent mother who wishes to bring up healthy children must discard them.' + '</i>' + '<br />' + '<br />' +
'Cow milk is expensive means it is',
options: ['sweet ',
 'white ',
 'good ',
 'strong ',
 'dear'],
answer: 4
},
{
question: '<b>' + 'Read Passage II carefully and answer the question which follow' + '</b>' + '<br />' + '<br />' + 'PASSAGE II' + '<br />' + '<br />' +
'<i>' + 'Apart from foods which supply us with energy, we need certain substances called protein to help us grow, and when we are fully grown, to maintain our strength. These proteins are found in meat, fish, eggs, milk, green vegetables and to a much lesser extent in grains like millet, wheat, guinea corn, rice, etc.' +
'Children fed chiefly on roots will, therefore, stop growing. They often get very ill and die while children who are given milk and eggs grow well and live longer.' +
'However, cow milk is expensive in West Africa and in many places there is a wrong tradition about eggs. Some old people say that if eggs are given to children they become liars. This is not true. Eggs do not make children either tell truth or lie. They are simply good food which will help the child to grow well. Also, the tradition that boys become thieves when they eat meat is not true. Both are bad traditions which have been repeated in some villages from one generation to another.' +
'So, the intelligent mother who wishes to bring up healthy children must discard them.' + '</i>' + '<br />' + '<br />' +
'What happens to children who are not given food containing proteins regularly?',
options: ['They often become liars.',
 'They often become thieves. ',
 'They often become ill.',
 'They should preserve tradition. ',
 'Their traditions often help them.'],
answer: 2
},
{
question: '<b>' + 'Read Passage II carefully and answer the question which follow' + '</b>' + '<br />' + '<br />' + 'PASSAGE II' + '<br />' + '<br />' +
'<i>' + 'Apart from foods which supply us with energy, we need certain substances called protein to help us grow, and when we are fully grown, to maintain our strength. These proteins are found in meat, fish, eggs, milk, green vegetables and to a much lesser extent in grains like millet, wheat, guinea corn, rice, etc.' +
'Children fed chiefly on roots will, therefore, stop growing. They often get very ill and die while children who are given milk and eggs grow well and live longer.' +
'However, cow milk is expensive in West Africa and in many places there is a wrong tradition about eggs. Some old people say that if eggs are given to children they become liars. This is not true. Eggs do not make children either tell truth or lie. They are simply good food which will help the child to grow well. Also, the tradition that boys become thieves when they eat meat is not true. Both are bad traditions which have been repeated in some villages from one generation to another.' +
'So, the intelligent mother who wishes to bring up healthy children must discard them.' + '</i>' + '<br />' + '<br />' +
'What advice did the writer give to mothers?',
options: ['They should eat foods containing proteins',
 'They should accept traditions',
 'They should do away with bad traditions',
 'They should preserve traditions',
 'They should feed their children on roots and grains only'],
answer: 2
},
{


question: '<b>' + 'Choose from the alternatives lettered A to D the one which is nearest in meaning to the underlined word in each sentence' + '</b>' + '<br />' + '<br />' + ' Adiza‟s mother prepares very ' + '<u>' + 'palatable' + '</u>' + ' dishes',
options: ['expensive',
 'rich',
 'tasty',
 'colourful',
 'interesting'],
answer: 2
},
{
question: '<b>' + 'Choose from the alternatives lettered A to D the one which is nearest in meaning to the underlined word in each sentence' + '</b>' + '<br />' + '<br />' + ' Sindi was ' + '<u>' + 'brought up' + '</u>' + ' by a very strict woman',
options: ['saved ',
 'reared ',
 'born ',
 'taught ',
 'beaten'],
answer: 1
},
{
question: '<b>' + 'Choose from the alternatives lettered A to D the one which is nearest in meaning to the underlined word in each sentence' + '</b>' + '<br />' + '<br />' + ' Janet promised to ' + '<u>' + 'attend to' + '</u>' + ' her mother-in-law who was ill.',
options: ['look after',
 'look at',
 'look into ',
 'look for ',
 'look on'],
answer: 0
},
{
question: '<b>' + 'Choose from the alternatives lettered A to D the one which is nearest in meaning to the underlined word in each sentence' + '</b>' + '<br />' + '<br />' + ' The printing company has over a thousand ' + '<u>' + 'agents' + '</u>' + ' here.',
options: ['vendors',
 'caretakers',
 'deputies',
 'correspondents',
 'representatives'],
answer: 4
},
{
question: '<b>' + 'Choose from the alternatives lettered A to D the one which is nearest in meaning to the underlined word in each sentence' + '</b>' + '<br />' + '<br />' + ' The police Inspector asked all drivers to ' + '<u>' + 'obey' + '</u>' + ' the speed-limit regulations',
options: ['understand',
 'notice',
 'recognize',
 'observe',
 'answer'],
answer: 3
},
{

question: '<b>' + 'In each of the following sentences a group of words has been underlined. Choose from the alternatives lettered A to D the one that best explains the underlined words' + '</b>' + '<br />' + '<br />' + ' It is very difficult for many workers to ' + '<u>' + 'make ends meet' + '</u>',
options: ['do two jobs at a time',
 'make workers and managers come together',
 'live within their means',
 'feed two people'],
answer: 2 
},
{
question: '<b>' + 'In each of the following sentences a group of words has been underlined. Choose from the alternatives lettered A to D the one that best explains the underlined words' + '</b>' + '<br />' + '<br />' + ' Since the thief came out of prison he has ' + '<u>' + 'turned over a new leaf.' + '</u>' + ' This means he has',
options: ['changed his style of stealing',
 'started growing flowers',
 'changed for the better',
 'grown even more stubborn',
 'got a new job'],
answer: 2
},
{
question: '<b>' + 'In each of the following sentences a group of words has been underlined. Choose from the alternatives lettered A to D the one that best explains the underlined words' + '</b>' + '<br />' + '<br />' + ' When the army stormed the village, all the inhabitants ' + '<u>' + 'took to their heels.' + '</u>' + ' This means ……..',
options: ['the army came to the village in the storm',
 'the inhabitants were frightened and stood still',
 'the inhabitants had their shoes removed',
 'the inhabitants danced around on their heels',
 'the inhabitants were frightened and so ran away'],
answer: 4
},
{
question: '<b>' + 'In each of the following sentences a group of words has been underlined. Choose from the alternatives lettered A to D the one that best explains the underlined words' + '</b>' + '<br />' + '<br />' + ' Berko is in ' + '<u>' + 'two minds' + '</u>' + ' about resigning from his job.' + '<br />' + 'This means that Berko has',
options: ['not really decided to resign ',
 'written his resignation letter ',
 'decided not to resign',
 'withdrawn his resignation letter',
 'been advised to resign'],
answer: 0
},
{
question: '<b>' + 'In each of the following sentences a group of words has been underlined. Choose from the alternatives lettered A to D the one that best explains the underlined words' + '</b>' + '<br />' + '<br />' + ' The teacher advised Kofi not to ' + '<u>' + 'beat about the bush' + '</u>' + ' This means Kofi was asked to ………….',
options: ['clear the bush',
 'beat others in the bush ',
 'go straight to the point ',
 'waste no more time',
 'stop clearing the bush'],
answer: 2
},
{

question: '<b>' + 'From the list of words lettered A to D, choose the one that is most nearly opposite in meaning to the word underlined in each sentence' + '</b>' + '<br />' + '<br />' + ' He ' + '<u>' + 'seldom' + '</u>' + ' comes here.',
options: ['never',
 'sometimes',
 'rarely ',
 'nearly ',
 'always'],
answer: 4
},
{
question: '<b>' + 'From the list of words lettered A to D, choose the one that is most nearly opposite in meaning to the word underlined in each sentence' + '</b>' + '<br />' + '<br />' + ' The police office was ' + '<u>' + ' harsh on' + '</u>' + ' the criminal',
options: ['soft to',
 'lenient with',
 'mild with',
 'cruel to',
 'rough'],
answer: 1
},
{
question: '<b>' + 'From the list of words lettered A to D, choose the one that is most nearly opposite in meaning to the word underlined in each sentence' + '</b>' + '<br />' + '<br />' + ' Musa was arrested for printing ' + '<u>' + 'counterfeit' + '</u>' + ' cedi notes',
options: ['correct',
 'new',
 'acceptable',
 'genuine',
 'fine'],
answer: 3
},
{
question: '<b>' + 'From the list of words lettered A to D, choose the one that is most nearly opposite in meaning to the word underlined in each sentence' + '</b>' + '<br />' + '<br />' + ' My father was a very ' + '<u>' + 'stout' + '</u>' + ' man',
options: ['handsome',
 'short ',
 'lean ',
 'ill',
 'weak'],
answer: 2
},
{
question: '<b>' + 'From the list of words lettered A to D, choose the one that is most nearly opposite in meaning to the word underlined in each sentence' + '</b>' + '<br />' + '<br />' + ' I ' + '<u>' + 'agreed to' + '</u>' + ' everything Lucy said.',
options: ['mocked at',
 'stood by',
 'interfered with',
 'took in',
 'objected to'],
answer: 4
},
{

question: '<b>' + 'From the alternatives lettered A to D, choose the one which most suitably completes each sentence.' + '</b>' + '<br />' + '<br />' + '  Help him to pay his fees, …………?',
options: ['will you ',
 'must you ',
 'can‟t you ',
 'won‟t you'],
answer: 0
},
{
question: '<b>' + 'From the alternatives lettered A to D, choose the one which most suitably completes each sentence.' + '</b>' + '<br />' + '<br />' + '  Kwasi, you are coming to see me tomorrow, ………….?',
options: ['isn‟t it',
 'aren‟t you',
 'won‟t you',
 'don‟t you'],
answer: 1
},
{
question: '<b>' + 'From the alternatives lettered A to D, choose the one which most suitably completes each sentence.' + '</b>' + '<br />' + '<br />' + '  If Mary had known she wouldn‟t have come, ……….?',
options: ['wasn‟t it ',
 'hadn‟t she ',
 'did she',
 'would she'],
answer: 3
},
{
question: '<b>' + 'From the alternatives lettered A to D, choose the one which most suitably completes each sentence.' + '</b>' + '<br />' + '<br />' + '  She dances so beautifully, …………..?',
options: ['not so',
 'can‟t she',
 'doesn‟t she',
 'isn‟t it'],
answer: 2
},
{
question: '<b>' + 'From the alternatives lettered A to D, choose the one which most suitably completes each sentence.' + '</b>' + '<br />' + '<br />' + '  JSS students work hard, ………?',
options: ['don‟t they',
 'shouldn‟t they',
 'can‟t they',
 'haven‟t they'],
answer: 0
},
{
question: '<b>' + 'From the alternatives lettered A to D, choose the one which most suitably completes each sentence.' + '</b>' + '<br />' + '<br />' + '  The chief with his linguists ………..coming to the durbar ground.',
options: ['is ',
 'are ',
 'were',
 'have been'],
answer: 0
},
{
question: '<b>' + 'From the alternatives lettered A to D, choose the one which most suitably completes each sentence.' + '</b>' + '<br />' + '<br />' + '  Most people find it hard to live up ………..their ideals',
options: ['by ',
 'to ',
 'for',
 'with'],
answer: 1
},
{
question: '<b>' + 'From the alternatives lettered A to D, choose the one which most suitably completes each sentence.' + '</b>' + '<br />' + '<br />' + '  The football match was put ……..until next week.',
options: ['in ',
 'out ',
 'off',
 'away'],
answer: 2
},
{
question: '<b>' + 'From the alternatives lettered A to D, choose the one which most suitably completes each sentence.' + '</b>' + '<br />' + '<br />' + '  These days, students are not keen ……..improving their English.',
options: ['with',
 'of',
 'about',
 'on'],
answer: 3
},
{
question: '<b>' + 'From the alternatives lettered A to D, choose the one which most suitably completes each sentence.' + '</b>' + '<br />' + '<br />' + '  When the plane arrived Kofi ………nowhere to be found.',
options: ['will be',
 'had been',
 'was',
 'would have been'],
answer: 2
},
{
question: '<b>' + 'From the alternatives lettered A to D, choose the one which most suitably completes each sentence.' + '</b>' + '<br />' + '<br />' + '  The boys swore that the books were ………..',
options: ['his',
 'theirs ',
 'their‟s ',
 'theirs‟'],
answer: 1
},
{
question: '<b>' + 'From the alternatives lettered A to D, choose the one which most suitably completes each sentence.' + '</b>' + '<br />' + '<br />' + '  Unless your father reports at the police station we …………all be whipped',
options: ['will',
 'shall',
 'should',
 'would'],
answer: 1
},
{
question: '<b>' + 'From the alternatives lettered A to D, choose the one which most suitably completes each sentence.' + '</b>' + '<br />' + '<br />' + '  The bell will go ………the next ten minutes',
options: ['between',
 'under',
 'from within',
 'within'],
answer: 3
},
{
question: '<b>' + 'From the alternatives lettered A to D, choose the one which most suitably completes each sentence.' + '</b>' + '<br />' + '<br />' + '  The heavy rains have ……….early this year.',
options: ['set in ',
 'set out ',
 'set up ',
 'set on'],
answer: 0
}
]