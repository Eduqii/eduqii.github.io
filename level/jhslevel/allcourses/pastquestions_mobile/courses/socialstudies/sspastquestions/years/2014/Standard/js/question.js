

//Array of Objects
const quiz = [
 {

question: ' Some Ghanaians mishandle the Cedi Notes by',

options: [' burning it',
 ' crumpling it',
 ' saving it',
 ' spending it'],

answer: 1


 
},

{

question: ' A major way of spreading HIV and AIDS is through',

options: [' eating in public places',
 ' indiscriminate sexual relations',
 ' mosquito bites',
 ' shaking hands with infected persons.'],

answer: 1
 

 
},

{

question: ' Adolescent abstinence from sexual related activity is',

options: [' chastity',
 ' health',
 ' paternity',
 ' sincerity'],


answer: 0
 
},

{

question: ' The set of guidelines for governing a nation is provided by the',

options: [' constitution',
 ' council of state',
 ' judiciary',
 ' legislature'],

answer: 0


 
},

{

question: ' What a person is obliged to perform as a duty in the society constitutes his / her',

options: [' achievements',
 ' dignity',
 ' responsibility',
 ' rights'],

answer: 2


 
},

{

question: ' The physical environment of Ghana can be protected through',

options: [' drying industrial waste',
 ' practicing traditional farming',
 ' redirecting river channels',
 ' recycling of waste materials'],

answer: 3

 
},

{

question: ' Promoting national unity in Ghana greatly depends on',

options: [' kindness',
 ' politics',
 ' religion',
 ' tolerance'],

answer: 3

 
},

{

question: ' The most effective measure for reducing the rapid population growth in Ghana is promoting',

options: [' adolescent counselling',
 ' women empowerment',
 ' gender equality',
 ' girl-child education'],

answer: 3

 
},

{

question: ' Which of the following situations is an effect of rapid population growth in Ghana?',

options: [' Decrease in food production',
 ' High dependency burden',
 ' High standard of living',
 ' Low productivity'],

answer: 1

 
},

{

question: ' Pressure on social facilities in urban areas is mainly created by',

options: [' decrease in imported goods',
 ' high cost of living',
 ' rural-urban drift',
 ' urban-rural drift'],

answer: 2
 

 
},

{

question: ' Community conflicts are best settled through',

options: [' arbitration and reconciliation',
 ' court ruling',
 ' government intervention',
 ' police action'],

answer: 0

 
},

{

question: ' Which of the following conditions is an effect of ethnic conflicts?',

options: [' Decrease in government expenditure',
 ' Frequent interruption of electricity supply',
 ' Interruption of development projects',
 ' Overdependence on foreign goods'],

answer: 2
 

 
},

{

question: ' Government policies are implemented at the District levels by the',

options: [' Chief Executive',
 ' Co-ordinating Director',
 ' Presiding Member',
 ' Regional Minister'],

answer: 0

 
},

{

question: ' Maintenance of law and order in the country is the main function of the',

options: [' airforce',
 ' army',
 ' navy',
 ' police'],

answer: 3

 
},

{

question: ' The highest authority in the traditional area is the',

options: [' clan head',
 ' divisional chiefs',
 ' lineage head',
 ' paramount chief'],

answer: 3

},

{

question: ' Which of the following factors promotes political stability in a country?',

options: [' Food security',
 ' Free education',
 ' Rigid constitution',
 ' Rule of law'],

answer: 3

 
},

{

question: ' Which of the following features is not an efficient way of sustaining political stability in a country?',

options: [' Avoidance of dictatorship',
 ' Free and fair elections',
 ' One party system',
 ' Press freedom'],

answer: 2
 


 
},

{

question: ' Poor drainage system in our cities result in',

options: [' earthquakes',
 ' erosion',
 ' floods',
 ' landslides'],

answer: 2
 

 
},

{

question: ' The Poll Tax Ordinance was passed in 1852 mainly to raise money to pay salaries of',

options: [' Christian missionaries',
 ' cocoa farmers',
 ' Colonial employees',
 ' Gold Coast chiefs'],

answer: 2


 
},

{

question: ' Which of the following activities is the most outstanding contribution of Ghana to the United Nations Organisation?',

options: [' Donation of relief items',
 ' Payment of dues',
 ' Settlement of refugees',
 ' Provision of troops for peace-keeping operations'],

answer: 3
 

},

{

question: ' Which of the following international organizations is the largest financial contributor to Ghana’s developmental projects?',

options: [' African Union (AU)',
 ' Economic Community of West African States (ECOWAS)',
 ' The Commonwealth',
 ' United Nations Organization.'],

answer: 3
 
},

{

question: ' The first political party formed in the Gold Coast was the',

options: [' Convention People’s Party (CPP)',
 ' National Liberation Movement (NLM)',
 ' United Gold Coast Convention (UGCC)',
 ' United Party (UP)'],

answer: 2
 

 
},

{

question: ' Which of the following settlements is a feature of a slum?',

options: [' Dispersed settlement',
 ' Linear settlement',
 ' Overcrowded settlement',
 ' Planned settlement'],

answer: 2

 
},

{

question: ' Cocoa production is classified as',

options: [' cottage industry',
 ' primary industry',
 ' secondary industry',
 ' tertiary industry'],

answer: 1
 

 
},

{

question: ' Pension benefits of Ghanaian workers are managed by the',

options: [' Bank of Ghana',
 ' Ghana Commercial Bank',
 ' Social Security and National Insurance Trust',
 ' State Insurance Company'],

answer: 2

 
},

{

question: ' Private businesses help in developing the nation when they',

options: [' increase their profit',
 ' import consumer goods',
 ' pay appropriate taxes',
 ' produce more goods'],

answer: 2

 
},

{

question: ' The main reason for establishing state owned enterprises is to',

options: [' create competition in the country.',
 ' generate foreign exchange for the country.',
 ' generate profit for the state.',
 ' provide essential service to the public'],

answer: 3
 
 
},

{

question: ' Landmarks are important features used in finding',

options: [' cardinal points',
 ' compass points',
 ' direction',
 ' layout'],

answer: 2

 
},

{

question: ' The main benefit of tourism to Ghana is that it',

options: [' encourages inter-tribal marriages',
 ' enriches our culture',
 ' is a major foreign exchange earner',
 ' promotes political stability'],

answer: 2

 
},

{

question: ' Ethnic conflicts must be avoided because they slow down',

options: [' economic development',
 ' rural-urban migration',
 ' tribal disintegration',
 ' urban-rural migration'],

answer: 0

 
},

{

question: ' A scale of map expressed as a ratio is known as',

options: [' linear scale',
 ' simple fraction',
 ' representative fraction',
 ' statement scale'],

answer: 2

 
},

{

question: ' The integrity of the nation can be defended by',

options: [' educating the youth',
 ' encouraging immigration',
 ' cleaning the environment',
 ' reporting saboteurs'],

answer: 3

 
},

{

question: ' One basic right of every Ghanaian is the right to',

options: [' fair wages',
 ' personal liberty',
 ' stand for election',
 ' vote in general election'],

answer: 1

 
},

{

question: ' Which of the following situations may not create instability in the Ghanaian society?',

options: [' Denial of justice',
 ' Gender equality',
 ' Party rivalry',
 ' Workers’ demonstration'],

answer: 1

 
},

{

question: ' The major reason for establishing castle schools by the Europeans was to',

options: [' educate the people on human rights',
 ' educate their own children',
 ' introduce formal education to the people',
 ' teach religious knowledge.'],

answer: 1

 
},

{

question: ' The main reason for the abolishing of the slave trade was the',

options: [' difficulties in the trade',
 ' inhuman nature of the trade.',
 ' lack of finance.',
 ' lack of market for slaves'],

answer: 1
 

 
},

{

question: ' Which of the following features forms part of our social environment?',

options: [' Atmosphere',
 ' Buildings',
 ' Land',
 ' Religion'],

answer: 3
 

 
},

{

question: ' Habitats of endangered species of animals can be protected through the creation of',

options: [' forest reserves',
 ' land guards',
 ' tourist centres',
 ' veterinary clinics'],


answer: 0

 
},

{

question: ' The most effective agency of cultural change is',

options: [' drama',
 ' education',
 ' language',
 ' music'],

answer: 1
 

 
},

{

question: ' The motto in Ghana’s Coat of Arms is',

options: [' Freedom and Love',
 ' Independence and Freedom.',
 ' Freedom and Justice',
 ' Unity and Integrity.'],

answer: 2
}
]